...
skipline(char c)
{
	while ((c = pli_read()) != TKN_NL);	/* skip comment */
}
	...
	while (!eoi) {
		...
		case _OPERATOR:
			...
			case _LOGIC:
				...
				case _BINARY:
					/*
					 * not an attribution and not quiet
					 * mode
					 */
					if (attrib == 0 && f.q == 0)
						printf("%c", c);
					...
					pushoprt(c);	/* push into operators
							 * stack */
					break;
		...
		default:	/* symbol belongs to no category. unknown
				 * symbol */
			break;
		}
		/* 2. Process */
