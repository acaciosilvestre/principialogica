...
int  main()
{
    ...
    _TOKENP _struct_t=&reg_token2[0]; 
    int fd;
    if((fd=open("tokens",O_RDWR | O_CREAT,0755))==-1){
	printf("mktoken tokens fail\n");
	exit(1);
	}
	write(fd,struct_t,sizeof(reg_token));
	write(fd,_struct_t,sizeof(reg_token2));
	return(0);
}
